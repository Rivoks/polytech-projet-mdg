...
@views function iceflow(dx, dy, Zbed, Hice, Mask, grad_b, z_ELA, b_max)
    ...
    while err>tolnl && iter<itMax
        ...
        S     .= B .+ H
        # error check
        if mod(iter, nout)==0 # on regarde tous les nout fois si on peut sortir
            @printf("gradS = %f, S = %f, D = %f, qH = %f, dtau = %f, ResH = %f, H = %f", gradS[50,80], S[50,80], D[50,80], qHx[50,80], dtau[50,80], ResH[50,80], H[50,80])
            Err .= Err .- H
