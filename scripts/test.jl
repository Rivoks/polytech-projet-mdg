...
if !@isdefined do_visu
    do_visu = true
end
if !@isdefined do_save
    do_save = true
end
...
@views function diffusion_1D(; do_visu = true)
    ...
    display(Mask1.data)
    Mask = Mask1.data[43, :] # Booléen qui nous permettra d'appliquer le masque pour le niveau de la mer
    ...
    if do_visu
        display(plot(yc, H0, linewidth = 3, label = "H initial"))
        display(plot!(yc, H, legend = true, framestyle = :box, linewidth = 3, label = "H apres diffusion", xlabel = "lx", ylabel = "H", title = "iceflow 1D (nt=$nout, iters=$iter)"))
